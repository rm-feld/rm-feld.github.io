...
$$\begin{align} W_{ij} = x_{ij}^{\intercal}\beta + x_{A, ij}^{\intercal}a_{i} + x_{B, ij}^{\intercal}b_{j} + \varepsilon _{ij} \end{align}$$
...
$$\begin{align} \mathbb{P}(W_{ij} > 0) &= \mathbb{P}\left( x_{ij}^{\intercal}\beta + x_{A, j}^{\intercal}a_{i}  + x_{B, j}^{\intercal} b_{j} + \varepsilon_{ij} \right) \\
&= \Phi\left( \frac{x_{ij}^{\intercal}\beta }{\sqrt{ x_{A, j}^{\intercal} \Sigma_{A} x_{A, j} + x_{B, j}^{\intercal} \Sigma_{B} x_{B, j} + 1}} \right),\end{align}$$
...
$$\begin{align} \mathbb{P}(Y_{i j} = y_{ij} \mid b) &= \mathbb{P}\left( x_{ij}^{\intercal}\beta + x_{A , j}a_{i} + x_{B, j}b_{j} + \varepsilon_{ij}  > 0 \mid b\right) \\
&= \Phi\left( \frac{x_{ij}^{\intercal}\beta  + x_{B, j}b_{j}}{\sqrt{ x_{A, j}^{2}\sigma^{2}_{A} + 1 }} \right).\end{align}$$
...
$$\begin{align}  \left\lvert\left\lvert b \right\rvert\right\rvert ^{2} = \sum_{j = 1}^{\tilde{J}} \frac{b_{j}^{2}}{x_{A, j}^{2} \sigma^{2}_{A} + 1} = \sum_{j =1 }^{\tilde{J}} \frac{\sigma_{B}^{2}}{x_{A, j}^{2} \sigma^{2}_{A} + 1} Z_j^{2},\end{align}$$
...
$$\begin{align} W_{ij} = \beta_{0} + x_{A, ij} a_{i} + x_{B, ij}b_{j} + \varepsilon_{ij}, \end{align}$$
so that marginally,
$$\begin{align} \mathbb{P}(Y_{ij} = 1) &= \Phi\left( \frac{\beta_{0}}{\sqrt{ x_{A_{i, j}}^{2} \sigma^{2}_{A} + x_{B_{i, j}}^{2} \sigma^{2}_{B}  + 1} } \right).\end{align}$$
...
$$\begin{align} W_{ij} \mid a_{i} \sim \mathcal{N}(\beta_{0} + x_{A, ij} a_{i} , \sigma^{2}_{B, ij} x_{B, ij}^{2} + 1), \end{align}$$
so if $x_{B, ij}$ is only dependent on $i$, then 
$$\begin{align} \mathbb{P}(Y_{ij} =1 \mid a_{i}) = \Phi\left( \frac{\beta_{0} + x_{A, ij} a_{i}}{\sqrt{ \sigma^{2}_{B} x_{B, i} ^{2} + 1} } \right) \end{align}$$
