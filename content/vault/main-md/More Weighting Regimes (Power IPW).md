...
$$\begin{align*} \frac{N}{R \cdot \max_{i}(N_{i \bullet }^{1 - \theta})} \leq \alpha \leq \frac{N}{R \cdot \min_{i}(N_{i \bullet }^{1 - \theta})} \end{align*}$$
so that, since we can reasonably assume $\min(N_{i \bullet}) = 2$ in the presented regimes, nets 

$$\begin{align*} \frac{N^{1 - \kappa_{1}}}{[\max_{i}N_{i}]^{1 - \theta}} \leq \alpha \leq \frac{N^{1 - \kappa_{1}}}{2^{1 - \theta}}. \end{align*}$$
May be over-reading, but the $\theta = \kappa_{1}$ bound feels appealing in this context. 
## Weight relations and bound
$$\begin{align*} \frac{\min_{i}N_{i \bullet }}{\max_{i} N_{i \bullet }} \leq \frac{\alpha}{N_{i \bullet }} \leq \frac{\max_{i}N_{i \bullet }}{\min_{i} N_{i \bullet }} \end{align*}$$
Note a crude bound
$$\begin{align*} N_{i\bullet }^{- \theta} \leq  \frac{1}{1 + \theta(x - 1)}. \end{align*}$$
we can also get 
$$\begin{align*} N_{i \bullet }^{-\theta} \leq \exp\left( -\frac{\theta (N_{i \bullet } - 1)}{N_{i \bullet }} \right). \end{align*}$$
# Conditions 
## Only Row Weighting
...
&\leq \frac{B^{2}\alpha^{2}}{N^{2}} \left( \sum_{i} N_{i \bullet }^{2 - 2\theta} + \sum_{ijr} \exp \left\{ -\theta [(N_{i\bullet} - 1)/N_{i\bullet } + (N_{r \bullet } - 1) / N_{r \bullet }] \right\} Z_{ij}Z_{rj} \right).\end{align*}$$
