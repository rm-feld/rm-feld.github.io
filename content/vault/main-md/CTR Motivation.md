...
$$\begin{align*} \max_{(i, j, k) \in \mathcal{S}} |S_{(i, j, k)}| m = O(N).   \end{align*}$$
Conservatively, we can write
$$\begin{align*} \max_{(i, j, k) \in \mathcal{S}} |S_{(i, j, k)}| &\leq \max_{(i, j, k) \in \mathcal{S}} \left\{ N_{i \bullet \bullet } + N_{\bullet  j \bullet } + N_{\bullet  \bullet  k} \right\}  \leq \max_{i}N_{i \bullet  \bullet } + \max_{j} N_{\bullet  j \bullet } + \max_{k} N_{\bullet \bullet  k}  \\ 
&= O\left( \max\left\{ \max_{i} N_{i \bullet \bullet }, \max_{j} N_{\bullet j \bullet }, \max_{k} N_{\bullet  \bullet  k} \right\}  \right). \end{align*}$$
...
$$\begin{align*} \max\left\{ \max_{i} N_{i \bullet  \bullet }, \max_{j} N_{\bullet  j \bullet }, \max_{k} N_{\bullet  \bullet  k} \right\} = O(N^{1 - \kappa_{1}}); \end{align*}$$
...
$$\begin{align*} \sum_{ijr} \frac{1}{N_{i \bullet }N_{r\bullet }} Z_{ij}Z_{rj} = o(N^{2\kappa_{1}})? \end{align*}$$
...
$$\begin{align*} \sum_{ijr} \frac{1}{N_{i \bullet } N_{r \bullet }}Z_{ij} Z_{rj} = \sum_{ir} \frac{|T_{i} \cap T_{r}|}{N_{i \bullet }N_{r\bullet }}. \end{align*}$$
...
$$\begin{align*} \sum_{ir} \frac{|T_{i} \cap T_{r}|}{N_{i \bullet }N_{r\bullet }} \approx \frac{N^{2\kappa}}{N^{2}} N^{2\kappa} \end{align*}$$
...
$$\begin{align*} \frac{ \partial g(\gamma_{0}) }{ \partial \gamma } = \frac{1}{N^{\kappa_{1}}} \sum_{ij} \frac{1}{N_{i \bullet }} Z_{ij}[x_{ij} - x_{ij}] = 0, \end{align*}$$
so $\gamma_{0}$ is a maximizer of $g$. Thus it suffices to ensure $g$ is strictly concave; writing 
$$\begin{align*} \frac{ \partial^{2} g(\gamma_{0}) }{ \partial \gamma \partial \gamma ^{\intercal} } = -\frac{1}{N^{\kappa_{1}}} \sum_{ij} \frac{1}{N_{i \bullet }} Z_{ij} \frac{\varphi \left( x_{ij}^{\intercal}\gamma_{0} \right)^{2}}{\Phi\left( x_{ij}^{\intercal}\gamma_{0} \right) \Phi\left( -x_{ij}^{\intercal}\gamma_{0} \right)}x_{ij}x_{ij}^{\intercal},  \end{align*}$$
...
$$\begin{align*} \text{Pr}\left\{P_{\tau^{2}_{A}}(y[1], y[2]) \mid y[1]\right\} \leq \frac{P_{\tau^{2}_{A} + \varepsilon}(y[1])}{P_{\tau^{2}_{A}}(y[1])};\end{align*}$$
...
$$\begin{align*} \mathbb{V}[\ell_{\text{all}}(\gamma)] \leq \frac{B^{2}}{N^{2}} \left( \sum_{ijs} \frac{Z_{ij}Z_{is}}{N_{i\bullet }^{2}N_{\bullet  j}N_{\bullet  s}}  + \sum_{ijr} \frac{Z_{ij}Z_{rj}}{N_{\bullet  j}^{2}N_{i\bullet }N_{r\bullet }}\right) \end{align*}$$

# Likelihood-Based Bashes
## All Likelihood
### Log Likelihood
$$\begin{align*} \ell_{\text{all}} = \sum_{(i, j, k) \in \mathcal{S}} \log \Phi\left( y_{ijk}x_{ijk}^{\intercal}\gamma \right) \end{align*}$$
### Score and Hessian
$$\begin{align*} \nabla_{\gamma} \ell_{\text{all}} = \sum_{(i, j, k) \in \mathcal{S}} y_{ijk}x_{ijk}\cdot \varphi \left( y_{ijk}x_{ijk}^{\intercal}\gamma \right) = \sum_{(i, j, k) \in \mathcal{S}} y_{ijk}x_{ijk}\cdot \varphi \left( x_{ijk}^{\intercal}\gamma \right)\end{align*}$$
$$\begin{align*} \nabla^{2}_{\gamma} \ell_{\text{all}} = -\sum_{(i ,j , k) \in \mathcal{S}} y_{ijk}^{2} \varphi \left( x_{ijk}^{\intercal}\gamma \right) \left[ x_{ijk}x_{ijk}^{\intercal} \right]\end{align*}$$
### Variance

$$\begin{align*} \mathbb{V}[\nabla_{\gamma} \ell_{\text{all}}] = \mathbb{E}\left[ \nabla_{\gamma}\ell_{\text{all}} \nabla_{\gamma} \ell_{\text{all}}^{\intercal} \right] - \mathbb{E}[\nabla_{\gamma} \ell_{\text{all}}]\mathbb{E}[\nabla_{\gamma} \ell_{\text{all}}]^{\intercal};\end{align*}$$
...
&+ \mathbb{P}\left( X_{ijk}^{\intercal}\beta + a_{i} + b_{j}  + c_{k} + \varepsilon_{ijk} < 0, X_{ist}^{\intercal}\beta + a_{i} + b_{s} + c_{t} + \varepsilon_{ist} < 0\right). \end{align*}$$
...
$$\begin{align*} \mathbb{P}(Y_{ijk} = Y_{ist}) = \Phi_{2}\left( X_{ijk}^{\intercal}\gamma_{0}, X_{ist}^{\intercal}\gamma_{0}; \rho_{A} \right) + \Phi_{2}\left( -X_{ijk}^{\intercal}\gamma_{0}, -X_{ist}^{\intercal}\gamma_{0}; \rho_{A} \right).\end{align*}$$
In turn, 
...
$$\begin{align*} \mathbb{E}[Y_{ijk}Y_{rjk}] = 4\Phi_{2} \left( X_{ijk}^{\intercal}\gamma_{0}, X_{rjk}^{\intercal}\gamma_{0}; \rho_{A} \right) -2\Phi\left( X_{ijk}^{\intercal}\gamma_{0} \right) - 2\Phi\left( X_{ijk}^{\intercal}\gamma_{0} \right) + 1. \end{align*}$$
Finally,
$$\begin{align*} \mathbb{E}[Y_{ijk}Y_{rst}] = 4\Phi\left( X_{ijk}^{\intercal}\gamma_{0} \right)\Phi\left( X_{rst}^{\intercal}\gamma_{0} \right) - 2\Phi\left( X_{ijk}^{\intercal}\gamma_{0} \right) - 2\Phi\left( X_{rst}^{\intercal}\gamma_{0} \right) + 1. \end{align*}$$
...
&+ \underbrace{ \sum_{ijkst} \varphi \left( x_{ijk}^{\intercal}\gamma \right) \varphi \left( x_{ist}^{\intercal}\gamma \right) \left(2\Phi\left( x_{ijk}^{\intercal}\gamma_{0} \right) - 1\right) \left(2\Phi\left( x_{ist}^{\intercal}\gamma_{0} \right) - 1\right)x_{ijk}x_{ist}^{\intercal} }_{ \text{II}^{*}(a) } + \text{II}^{*}(b) + \text{II}^{*}(c)  \\ 
&+ \underbrace{ \sum_{ijkr} \varphi \left( x_{ijk}^{\intercal}\gamma \right)\varphi \left( x_{rjk}^{\intercal}\gamma \right)\left(2\Phi\left( x_{ijk}^{\intercal}\gamma_{0} \right) - 1\right)\left(2\Phi\left( x_{rjk}^{\intercal}\gamma_{0} \right) - 1\right)x_{ijk}x_{rjk}^{\intercal} }_{ \text{III}^{*}(a) } + \text{III}^{*}(b) + \text{III}^{*}(c) \\ 
...
&+ \sum_{ijkr} \varphi\left( x_{ijk}^{\intercal}\gamma \right) \varphi \left( x_{rjk}^{\intercal}\gamma \right)\left[ \Phi_{2}\left( x_{ijk}^{\intercal}\gamma, x_{rjk}^{\intercal}\gamma; \rho_{-A} \right) - \Phi\left( x_{ijk}^{\intercal}\gamma \right)\Phi\left( x_{rjk}^{\intercal}\gamma \right) \right] x_{ijk}x_{rjk}^{\intercal} + \text{III}(b) + \text{III}(c).  \end{align*}$$
